...
For linked Git worktrees, Codex must resolve the target path from `git worktree list --porcelain -z` and pass that absolute path through the command tool's structured working-directory field. When one shell step must address multiple worktrees, use a NUL-aware parser and quote each resolved path, for example `git -C "$worktree_path" …`. Opening Terminal or asking the user to locate a checkout is not a worktree-selection mechanism.
